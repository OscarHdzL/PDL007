...
{
    public class InsertarTomaNotaRepresentanteAccesoDatos : BaseAccesoDatos
    {
        #region SP_Operaciones
        private const string sp_insertar_usuario_sistema = "religiosos.sp_insertar_representante_toma_nota";
        #endregion
        ...
        /// </summary>
        public InsertarTomaNotaRepresentanteAccesoDatos()
            : base() { }
        #endregion
        ...
        /// <returns></returns>
        private List<EntidadParametro> ObtenerParametros(InsertarTomaNotaRepresentanteRequest request)
        {
        ...
        {
            List<InsertarTomaNotaRepresentanteResponse> respuesta = new();
            try
            {
                using (var conexion = new Contexto())
                {
                    switch (int.Parse(Configuration["TipoBase"].ToString()))
                    {
                        case 1:
                            ...
                            respuesta = await conexion.InsertarTomaNotaRepresentanteResponse.FromSqlRaw(resulMySQL.Query, resulMySQL.ListaParametros.ToArray()).ToListAsync();
                            break;
                        ...
                        case 2:
                            ...
                            respuesta = await conexion.InsertarTomaNotaRepresentanteResponse.FromSqlRaw(resulPostgreSQL.Query, resulPostgreSQL.ListaParametros.ToArray()).ToListAsync();
                            break;
                    }
                }

                return new ResponseGeneric<List<InsertarTomaNotaRepresentanteResponse>>(respuesta);
            }
            catch (Exception ex)
            {
