...
namespace Acceso_Datos.Operaciones
{
    ...
    /// </summary>
    public class InsertarTramitePasoUnoAccesoDatos : BaseAccesoDatos
    {
        #region SP_Operaciones
        private const string sp_insertar_usuario_sistema = "religiosos.sp_insertar_tramite_paso_uno";
        #endregion
        ...
        /// </summary>
        public InsertarTramitePasoUnoAccesoDatos()
            : base() { }
        #endregion
        ...
        /// <returns></returns>
        private List<EntidadParametro> ObtenerParametros(InsertarTramitePasoUnoRequest request)
        {
            return new List<EntidadParametro>
            {
                 new EntidadParametro { Nombre = "S_US_ID", Tipo = "Int", Valor = request.s_id_us },
                 ...
                 new EntidadParametro { Nombre = "C_MATRIZ", Tipo = "String", Valor = request.c_matriz ?? "NULL"  },
            };
        }
        #endregion
        ...
        /// <returns></returns>
        public async Task<ResponseGeneric<List<InsertarTramitePasoUnoResponse>>> Operacion(InsertarTramitePasoUnoRequest request)
        {
            List<InsertarTramitePasoUnoResponse> respuesta = new();
            try
            {
                using (var conexion = new Contexto())
                {
                    switch (int.Parse(Configuration["TipoBase"].ToString()))
                    {
                        case 1:
                            ...
                            respuesta = await conexion.InsertarTramitePasoUnoResponse.FromSqlRaw(resulMySQL.Query, resulMySQL.ListaParametros.ToArray()).ToListAsync();
                            break;
                        ...
                        case 2:
                            ...
                            respuesta = await conexion.InsertarTramitePasoUnoResponse.FromSqlRaw(resulPostgreSQL.Query, resulPostgreSQL.ListaParametros.ToArray()).ToListAsync();
                            break;
                    }
                }

                return new ResponseGeneric<List<InsertarTramitePasoUnoResponse>>(respuesta);
            }
            catch (Exception ex)
            {
                LogErrores("InsertarTramitePasoUnoAccesoDatos", ex);
                throw;
            }
        }
        #endregion
