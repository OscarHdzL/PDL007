...
{
    public class ActualizarCatalogoCredoAccesoDatos : BaseAccesoDatos
    {
        #region SP_Operaciones
        private const string sp_Actualizar_convocatoria = "religiosos.sp_actualizar_catalogo_credo";
        #endregion
        ...
        /// </summary>
        public ActualizarCatalogoCredoAccesoDatos()
            : base() { }
        #endregion
        ...
        /// <returns></returns>
        private List<EntidadParametro> ObtenerParametros(ActualizarCatalogoCredoRequest entidad)
        {
        ...
        /// <returns></returns>
        public async Task<ResponseGeneric<List<ActualizarCatalogoCredoResponse>>> Operacion(ActualizarCatalogoCredoRequest request)
        {
            List<ActualizarCatalogoCredoResponse> respuesta = new List<ActualizarCatalogoCredoResponse>();
            try
            {
                using (var conexion = new Contexto())
                {
                    switch (int.Parse(Configuration["TipoBase"].ToString()))
                    {
                        case 1:
                            ...
                            respuesta = await conexion.ActualizarCatalogoCredoResponse.FromSqlRaw(resulMySQL.Query, resulMySQL.ListaParametros.ToArray()).ToListAsync();
                            break;
                        ...
                        case 2:
                            ...
                            respuesta = await conexion.ActualizarCatalogoCredoResponse.FromSqlRaw(resulPostgreSQL.Query, resulPostgreSQL.ListaParametros.ToArray()).ToListAsync();
                            break;
                    }
                }

                return new ResponseGeneric<List<ActualizarCatalogoCredoResponse>>(respuesta);
            }
            catch (Exception ex)
            {
