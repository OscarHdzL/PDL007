...
CREATE OR REPLACE FUNCTION religiosos.sp_consulta_lista_registros_transmisiones_dict(
	id_usuario integer,
    ...
    RETURNS TABLE(numero_registro bigint, id_transmision integer, folio character varying, domicilio character varying, numero_sgar character varying, denominacion character varying, representante character varying, id_estatus integer, estatus character varying, fecha_solicitud date, fecha_autorizacion date) 
    LANGUAGE 'plpgsql'
    COST 100
...
IF estatus_desc =33 OR estatus_desc=34 THEN
	...
	SELECT  
			    ...
			    FROM religiosos."TBL_Transmision" AS tbl_transmision 
                				 LEFT JOIN religiosos."ASOC_TransmisionDictaminador" AS tbl_asoc_transmision ON tbl_transmision.i_id = tbl_asoc_transmision.i_id_tbl_transmision
		       ...
		       JOIN religiosos."CAT_Estatus" tbl_estatus ON tbl_estatus.i_id= astes.i_id_tbl_estatus
				 ...
				 WHERE  tbl_asoc_transmision.id_tbl_usuario_dictaminador = id_usuario 
		       ...
		       JOIN religiosos."CAT_Estatus" tbl_estatus ON tbl_estatus.i_id= astes.i_id_tbl_estatus
				 ...
				 WHERE tbl_asoc_transmision.id_tbl_usuario_dictaminador = id_usuario 
				  ...
				  AND ((representante_desc IS null ) OR  (LOWER(tbl_transmision.c_representante) LIKE ('%'||LOWER(representante_desc)||'%')))
				 	 AND astes.i_id_tbl_estatus = (SELECT i_id_tbl_estatus FROM religiosos."ASOC_Transmision_Estatus"
									  WHERE i_id_tbl_transmision=tbl_transmision.i_id ORDER BY i_id DESC limit 1 )
									  AND astes.i_id_tbl_estatus in (30,31,32,38, 33, 34);
				  END IF;
