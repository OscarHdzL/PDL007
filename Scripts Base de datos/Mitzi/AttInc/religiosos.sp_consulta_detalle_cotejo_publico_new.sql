-- FUNCTION: religiosos.sp_consulta_detalle_cotejo_publico_new(integer, integer)

-- DROP FUNCTION IF EXISTS religiosos.sp_consulta_detalle_cotejo_publico_new(integer, integer);
...
IF(status in (3,4,5,6,7,8,14,15,16,17,18,19,20)) THEN
		 ...
		 UPDATE religiosos."ASOC_TraDictaminador" SET b_read = true
		 WHERE i_id_tbl_tramite = id_tramite;
...
ALTER FUNCTION religiosos.sp_consulta_detalle_cotejo_publico_new(integer, integer)
