-- FUNCTION: religiosos.sp_consulta_lista_registros_tnota_i(integer, character varying, character varying, integer)

-- DROP FUNCTION IF EXISTS religiosos.sp_consulta_lista_registros_tnota_i(integer, character varying, character varying, integer);

CREATE OR REPLACE FUNCTION religiosos.sp_consulta_lista_registros_tnota_i(
	id_usuario integer,
	denominacion_desc character varying,
	nombre_desc character varying,
	estatus_desc integer)
    RETURNS TABLE(id_tramite integer, id_tnota integer, nombre character varying, denominacion character varying, folio character varying, fecha_registro date, tramite_solicitado character varying, id_estatus integer, estatus character varying, fecha_autorizacion date, notificacion_read boolean) 
    LANGUAGE 'plpgsql'
    COST 100
...
IF estatus_desc= 27 OR estatus_desc=28 THEN
	RETURN QUERY
	SELECT 	
			DISTINCT tbl_tramite.i_id AS id_tramite,
			...
			(SELECT tblf.d_inicio FROM religiosos."ASOC_Tramite_Fechas" asoctf 
				INNER JOIN religiosos."TBL_Fechas" AS tblf ON asoctf.i_id_tbl_fechas = tblf.i_id
				WHERE asoctf.i_id_tbl_tramite = tbl_tramite.i_id and tblf.i_id_cat_fechas = 5 limit 1) AS fecha_autorizacion,
			(SELECT coalesce(b_read, true) FROM religiosos."ASOC_TnotaDictaminador"
			  WHERE i_id_tbl_tnota = tbl_tnota_tramite.i_id_tbl_tnota) AS notificacion_read		
		FROM 
			religiosos."TBL_Tramite" AS tbl_tramite
			INNER JOIN religiosos."ASOC_Tramite_Usuario" AS tbl_asoc_tramite ON tbl_tramite.i_id = tbl_asoc_tramite.i_id_tbl_tramite
			INNER JOIN religiosos."ASOC_TramTNota" AS tbl_tnota_tramite ON  tbl_tramite.i_id = tbl_tnota_tramite.i_id_tbl_tramite 
		...
		WHERE  tbl_asoc_tramite.i_id_tbl_usuario = id_usuario
			...
			AND tbl_tramite.c_denominacion <> '' AND tbl_tramite.c_nregistro <> ''
...
ELSE
	RETURN QUERY
		...
		FROM 
			...
			AND ((estatus_desc IS null ) OR ( tbl_estatus.i_id=estatus_desc ))
			AND tbl_tramite.c_denominacion <> '' AND tbl_tramite.c_nregistro <> ''
			AND tbl_estatus.i_id in (12,13,21,22,23,24,25,26,37, 27, 28);
		END IF;
END
$BODY$;

ALTER FUNCTION religiosos.sp_consulta_lista_registros_tnota_i(integer, character varying, character varying, integer)
    OWNER TO postgres;
