-- FUNCTION: religiosos.sp_consulta_lista_registros_transmisiones(integer, character varying, character varying, integer, character varying)

-- DROP FUNCTION IF EXISTS religiosos.sp_consulta_lista_registros_transmisiones(integer, character varying, character varying, integer, character varying);
...
CREATE OR REPLACE FUNCTION religiosos.sp_consulta_lista_registros_transmisiones(
    ...
    RETURNS TABLE(numero_registros bigint, id_transmision integer, folio character varying, domicilio character varying, numero_sgar character varying, denominacion character varying, representante character varying, id_estatus integer, estatus character varying, fecha_solicitud date, fecha_autorizacion date, notificacion_read boolean) 
    LANGUAGE 'plpgsql'
    COST 100
...
RETURN QUERY
	SELECT 
				...
				coalesce((SELECT b_read FROM religiosos."TBL_Observaciones_Transmision"
				WHERE i_id_tbl_transmision = tbl_transmision.i_id ORDER BY i_id DESC
				LIMIT 1), true) as notificacion_read
...
ALTER FUNCTION religiosos.sp_consulta_lista_registros_transmisiones(integer, character varying, character varying, integer, character varying)
