-- FUNCTION: religiosos.sp_consulta_detalle_transmision_cotejo_publico_new(integer, integer)

-- DROP FUNCTION IF EXISTS religiosos.sp_consulta_detalle_transmision_cotejo_publico_new(integer, integer);
...
ORDER BY asocte.i_id DESC limit 1;
	...
	IF(status in (29,30,31,32,33, 34)) THEN
		RETURN QUERY 
			...
			UPDATE religiosos."TBL_Observaciones_Transmision" SET b_read = true
			WHERE i_id_tbl_transmision = id_trans AND b_read = false;
	ELSE
	    RETURN;
	END IF;
END;
...
ALTER FUNCTION religiosos.sp_consulta_detalle_transmision_cotejo_publico_new(integer, integer)
