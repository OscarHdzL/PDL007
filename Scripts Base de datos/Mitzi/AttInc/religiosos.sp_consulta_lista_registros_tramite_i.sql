-- FUNCTION: religiosos.sp_consulta_lista_registros_tramite_i(integer, character varying, character varying, integer, integer)

-- DROP FUNCTION IF EXISTS religiosos.sp_consulta_lista_registros_tramite_i(integer, character varying, character varying, integer, integer);
...
CREATE OR REPLACE FUNCTION religiosos.sp_consulta_lista_registros_tramite_i(
	...
	credo_desc integer)
    RETURNS TABLE(id_registro integer, folio character varying, nombre character varying, denominacion character varying, fecha_registro date, fecha_autorizacion date, credo character varying, sol_registro character varying, id_estatus integer, estatus character varying, notificacion_read boolean) 
    LANGUAGE 'plpgsql'
    COST 100
...
IF estatus_desc= 19 OR estatus_desc=20 THEN
	...
	(SELECT coalesce(b_read, true) FROM religiosos."ASOC_TraDictaminador"
	 WHERE i_id_tbl_tramite = tblt.i_id) AS notificacion_read		
FROM
...
ALTER FUNCTION religiosos.sp_consulta_lista_registros_tramite_i(integer, character varying, character varying, integer, integer)
