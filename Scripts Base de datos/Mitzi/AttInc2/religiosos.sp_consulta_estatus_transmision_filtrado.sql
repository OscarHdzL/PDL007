-- FUNCTION: religiosos.sp_consulta_estatus_transmision_filtrado(integer, integer, integer, character varying)

-- DROP FUNCTION IF EXISTS religiosos.sp_consulta_estatus_transmision_filtrado(integer, integer, integer, character varying);
...
CREATE OR REPLACE FUNCTION religiosos.sp_consulta_estatus_transmision_filtrado(
    ...
    RETURNS TABLE(id_tbl_transmision integer, c_numero_sgar character varying, c_denominacion character varying, c_fecha character varying, id_tbl_estatus integer, c_estatus character varying, i_id_tbl_dictaminador integer, nombre_dictaminador character varying) 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
	...
	IF(id_estatus = 29) THEN
		RETURN QUERY 
		SELECT tbl_total.i_id_tbl_transmision,
		   tbl_total.c_numero_sgar,
		   tbl_total.c_denominacion,
		   tbl_total. c_fecha,
		   tbl_total.i_id_tbl_estatus,
		   tbl_total.c_estatus, 
		   tbl_total.i_id_tbl_dictaminador,
		   tbl_total.nombre_dictaminador 
		FROM (
			SELECT 
			...
			FROM 
				...
				(SELECT i_id_tbl_estatus, i_id_tbl_transmision FROM religiosos."ASOC_Transmision_Estatus"
					WHERE i_id in (SELECT MAX (i_id) FROM religiosos."ASOC_Transmision_Estatus" GROUP BY i_id_tbl_transmision) 
					order by i_id_tbl_transmision desc) as asocte on tblt.i_id = asocte.i_id_tbl_transmision INNER JOIN
			...
			FROM 
				...
				religiosos."ASOC_TransmisionDictaminador" asoctd ON tblt.i_id = asoctd.i_id_tbl_transmision LEFT JOIN
				religiosos."TBL_Usuario" as tblu ON asoctd.id_tbl_usuario_dictaminador = tblu.i_id 	
			WHERE cate.i_id in (30,31,32,33,34,38)
			AND asoctd.id_tbl_usuario_asignador = id_asignador
			AND 
			(busqueda IS NULL OR lower(tblt.c_numero_sgar) LIKE CONCAT('%',lower(busqueda),'%') OR lower(tblt.c_denominacion) LIKE CONCAT('%',lower(busqueda),'%') 
				OR lower(cate.nombre) LIKE CONCAT('%',lower(busqueda),'%') OR lower(tblu.c_usuario) LIKE CONCAT('%',lower(busqueda),'%')
			)
	...
	ELSEIF (id_dictaminador <> 0) THEN
		...
		, CASE WHEN est.i_id_tbl_estatus IN (29, 30) THEN CAST('En Proceso' AS character varying)
			   ...
			   WHEN est.i_id_tbl_estatus IN (38) THEN CAST('Observación atendida' AS character varying)
...
ALTER FUNCTION religiosos.sp_consulta_estatus_transmision_filtrado(integer, integer, integer, character varying)
