-- FUNCTION: religiosos.sp_consulta_declaratoria_lista_busqueda(integer, integer, character varying, character varying, integer)

-- DROP FUNCTION IF EXISTS religiosos.sp_consulta_declaratoria_lista_busqueda(integer, integer, character varying, character varying, integer);

CREATE OR REPLACE FUNCTION religiosos.sp_consulta_declaratoria_lista_busqueda(
	p_id_usuario integer,
	p_id_rol integer,
	p_denominacion character varying,
	p_folio character varying,
	p_estatus integer)
    RETURNS TABLE(id_declaratoria integer, folio character varying, denominacion_religiosa character varying, id_estatus integer, estatus character varying, fecha_envio character varying, fecha_autorizacion character varying, comentarios character varying, id_dictaminador integer, correo_dictaminador text, nombre_dictaminador text, notificacion_read boolean) 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
	...
	IF(p_id_rol = 0 AND p_id_usuario <> 0) THEN --Usuario
		RETURN QUERY 
		SELECT declaratoria.i_id as id_declaratoria
			...
			coalesce(asoc.b_read, true) AS notificacion_read
		...
		AND declaratoria.i_id_tbl_estatus IN (39, 40, 41, 42, 43, 44, 45, 46, 47) AND declaratoria.i_id_tbl_usuario = p_id_usuario;
	
	ELSEIF (p_id_rol = 11) THEN
		RETURN QUERY 
		SELECT declaratoria.i_id as id_declaratoria
			, declaratoria.c_folio as folio
			, declaratoria.c_denominacion_religiosa as denominacion_religiosa
		...
		AND ((p_folio IS null) OR (LOWER(declaratoria.c_folio) LIKE ('%'||LOWER(p_folio)||'%'))) 
		AND ((p_estatus IS null) OR (p_estatus = declaratoria.i_id_tbl_estatus ))
		AND declaratoria.i_id_tbl_estatus IN (40)
		...
		AND ((p_folio IS null) OR (LOWER(declaratoria.c_folio) LIKE ('%'||LOWER(p_folio)||'%'))) 
		AND ((p_estatus IS null) OR (p_estatus = declaratoria.i_id_tbl_estatus ))
		AND declaratoria.i_id_tbl_estatus IN (41, 42, 43, 44, 45, 46, 47) AND asoc.id_tbl_asignador = p_id_usuario;
	
	ELSEIF (p_id_rol = 12) THEN --Dictaminador
		RETURN QUERY
		SELECT declaratoria.i_id as id_declaratoria
			, declaratoria.c_folio as folio
			, declaratoria.c_denominacion_religiosa as denominacion_religiosa
		...
		AND ((p_folio IS null) OR (LOWER(declaratoria.c_folio) LIKE ('%'||LOWER(p_folio)||'%'))) 
		AND ((p_estatus IS null) OR (p_estatus = declaratoria.i_id_tbl_estatus ))
		AND declaratoria.i_id_tbl_estatus IN (41, 42, 43, 44, 45, 46, 47) AND (asoc.id_tbl_dictaminador = p_id_usuario or p_id_usuario = 0);
	END IF;
...
ALTER FUNCTION religiosos.sp_consulta_declaratoria_lista_busqueda(integer, integer, character varying, character varying, integer)
