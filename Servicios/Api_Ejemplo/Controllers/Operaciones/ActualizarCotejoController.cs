...
namespace Religiosos_Api.Controllers.Operaciones
{
    ...
    [Authorize]
    public class ActualizarCotejoController : Controller
    {
        #region Propiedades
        private readonly ActualizarCotejoNegocio _negocio;
        ...
        /// </summary>
        public ActualizarCotejoController(IConfiguration configuration)
        {
            _negocio = new ActualizarCotejoNegocio();
        ...
        public async Task<IActionResult> Post([FromBody] ActualizarCotejoRequest request)
        {
                        ...
                        if ((bool)resultado.Response[0]?.proceso_existoso)
                        {
                            ...
                            if (usuario.Response.Count > 0)
                            {
                                ...
                                String bodyCorreo = "<HTML style='padding:20px;'><head><style>a:hover{ background-color: #828282!important; }</style></head><h1> Estimado(a):  " + ToemailAddress.Name + "</h1><br>" +
                                                    "<div>Por favor, ingrese a su cuenta para dar seguimiento a su trámite de registro de asociación religiosa.</div><br>" +
            ...
            {
                log.LogError("ActualizarCotejoController - Post", ex);
