...
namespace Asuntos_Religiosos_Api.Controllers.Operaciones
{
    ...
    [Authorize]
    public class ActualizarEstatusTransmisionController : Controller
    {
        #region Propiedades
        private readonly ActualizarEstatusTransmisionNegocio _negocio;
        ...
        #region Constructor
        public ActualizarEstatusTransmisionController(IConfiguration configuration)
        {
            _negocio = new ActualizarEstatusTransmisionNegocio();
        ...
        [HttpPost("[action]")]
        public async Task<IActionResult> Post([FromBody] ActualizarEstatusTransmisionRequest request)
        {
            try
            {
                ...
                if (resultado.Status == ResponseStatus.Success)
                {
                    ...
                    {
                        var usuario = await _negocioGeneral.ConsultaUsuPerfil(new ConsultaDetalleUsuarioSistemaRequest { id_usuario = 5 });
                        ...
                        if (usuario.Response.Count > 0)
                        {
                            foreach (var item in usuario.Response)
                            {
                                ...
                                String bodyCorreo = "<HTML style='padding:20px;'><head><style>a:hover{ background-color: #828282!important; }</style></head><h1> Estimado(a)  " + ToemailAddress.Name + "</h1><br>" +
                                                "<div>Su registro se generó de forma exitosa</div><br></HTML>";
            ...
            {
                log.LogError("ActualizarEstatusTransmisionController - Post", ex);
