...
namespace Asuntos_Religiosos_Api.Controllers.Operaciones
{
    ...
    [Authorize]
    public class AsignarDictaminadorTomaNotaController : Controller
    {
        #region Propiedades
        private readonly AsignarDictaminadorTomaNotaNegocio _negocio;
        ...
        /// </summary>
        public AsignarDictaminadorTomaNotaController(IConfiguration configuration)
        {
            _negocio = new AsignarDictaminadorTomaNotaNegocio();
        ...
        [HttpPost("[action]")]
        public async Task<IActionResult> Post([FromBody] AsignarDictaminadorTomaNotaRequest[] request)
        {
            try
            {
                            ...
                            if (usuario.Response.Count > 0)
                            {
                                foreach (var item in usuario.Response)
                                {
                                    ...
                                    String bodyCorreo =
                                       ...
                                       "<div>Ha recibido una solicitud de toma de nota.</div><br>" +
                                       "</HTML>";
            ...
            {
                log.LogError("AsignarDictaminadorTomaNotaController - Post", ex);
