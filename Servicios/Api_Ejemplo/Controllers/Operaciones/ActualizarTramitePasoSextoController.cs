...
namespace Religiosos_Api.Controllers.Operaciones
{
    ...
    [Authorize]
    public class ActualizarTramitePasoSextoController : Controller
    {
        #region Propiedades
        private readonly ActualizarTramitePasoSextoNegocio _negocio;
        ...
        /// </summary>
        public ActualizarTramitePasoSextoController(IConfiguration configuration)
        {
            _negocio = new ActualizarTramitePasoSextoNegocio();
        ...
        [HttpPost("[action]")]
        public async Task<IActionResult> Post([FromBody] ActualizarTramitePasoSextoRequest request)
        {
            try
            ...
            {
                log.LogError("ActualizarTramitePasoSextoController - Post", ex);
        ...
        [HttpPost("[action]")]
        public async Task<IActionResult> Finalizar([FromBody] ActualizarTramitePasoSextoRequest request)
        {
            try
            
            {
                ...
                if (resultado.Status == ResponseStatus.Success)
                {
                    if (!string.IsNullOrEmpty(resultado.respuesta) || resultado.Response.Count > 0)
                    {
                        ...
                        {
                            var usuario = await _negocio.ConsultaUsuPerfil(new ConsultaDetalleUsuarioSistemaRequest {id_usuario = 7} );
                            ...
                            if (usuario.Response.Count > 0)
                            {
                                foreach (var item in usuario.Response)
                                {
                                    ...
                                    String bodyCorreo = "<HTML style='padding:20px;'><head><style>a:hover{ background-color: #828282!important; }</style></head><h1> Estimado(a):  " + ToemailAddress.Name + "</h1><br>" +
                                                        "<div>Por favor, ingrese a su cuenta para dar seguimiento a su trámite de registro.</div><br>" +
