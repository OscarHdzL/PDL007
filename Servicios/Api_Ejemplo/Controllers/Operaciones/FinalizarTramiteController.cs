...
namespace Religiosos_Api.Controllers.Operaciones
{
    ...
    [Authorize]
    public class FinalizarTramiteController : Controller
    {
        #region Propiedades
        private readonly FinalizarTramiteNegocio _negocio;
        ...
        /// </summary>
        public FinalizarTramiteController(IConfiguration configuration)
        {
            _negocio = new FinalizarTramiteNegocio();
        ...
        [HttpPost("[action]")]
        public async Task<IActionResult> Post([FromBody] FinalizarTramiteRequest request)
        {
            try
            
            {
                ...
                if (resultado.Status == ResponseStatus.Success)
                {
                    ...
                    {
                        if (resultado.Response[0].proceso_existoso)
                        {
                            var usuario = await _negocio.Consulta(new ConsultaDetalleUsuarioSistemaRequest { id_usuario = request.s_id_us });
                            ...
                            if (usuario.Response.Count > 0)
                            {
                                ...
                                String bodyCorreo = "<HTML style='padding:20px;'><head><style>a:hover{ background-color: #828282!important; }</style></head><h1> Estimado(a)  " + ToemailAddress.Name + "</h1><br>" +
                                                    "<div>Su registro se generó de forma exitosa, en breve será notificado sobre el estatus de su registro</div><br></HTML>";
            ...
            {
                log.LogError("FinalizarTramiteController - Post", ex);
