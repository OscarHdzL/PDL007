...
namespace Asuntos_Religiosos_Api.Controllers.Operaciones
{
    ...
    [Authorize]
    public class FinalizarTomaNotaController : Controller
    {
        #region Propiedades
        private readonly FinalizarTomaNotaNegocio _negocio;
        ...
        /// </summary>
        public FinalizarTomaNotaController(IConfiguration configuration)
        {
            _negocio = new FinalizarTomaNotaNegocio();
        ...
        [HttpPost("[action]")]
        public async Task<IActionResult> Post([FromBody] FinalizarTomaNotaRequest request)
        {
            try

            {
                ...
                if (resultado.Status == ResponseStatus.Success)
                {
                    ...
                    {
                        var usuario = await _negocioGeneral.ConsultaUsuPerfil(new ConsultaDetalleUsuarioSistemaRequest { id_usuario = 8 });
                        ...
                        if (usuario.Response.Count > 0)
                        {
                            foreach (var item in usuario.Response)
                            {
                                ...
                                String bodyCorreo = "<HTML style='padding:20px;'><head><style>a:hover{ background-color: #828282!important; }</style></head><h1> Estimado(a)  " + ToemailAddress.Name + "</h1><br>" +
                                                "<div>Su registro Toma de Nota se generó de forma exitosa, en breve será notificado sobre el estatus de su registro</div><br></HTML>";
            ...
            {
                log.LogError("FinalizarTomaNotaController - Post", ex);
